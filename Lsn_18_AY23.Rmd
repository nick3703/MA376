...
```{r}
single.mean <- lm(GradRate~1,data=Graduation)
summary(single.mean)

```

Mean is $\beta_0=41.98$, and $\sigma=24.39$, for the model:
\begin{align*}
...
```{r}
our.lm<-lm(GradRate ~ Tuition+Type,data=Graduation)
summary(our.lm)
```
...
24. Summarize your conclusions from the model as they relate to the original statistical question.