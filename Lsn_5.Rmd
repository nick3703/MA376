...
j & = \mbox{Scent status}\\
i & = \mbox{Student}\\
y_{i,j}& = \mbox{Store rating from the }i\mbox{th observation exposed to the }j\mbox{th scent}\\
...
From here we also obtained SSTotal, $\sum_{i,j} \left(y_{i,j}-\bar{y}\right)^2$

```{r}
sum((scent.dat$rating-mean(scent.dat$rating))^2)
```
Which can be decomposed into SSGroups (or SSModel) and SSError.  Recall SSE=$\sum_{i,j} \left(y_{i,j}-\hat{y_{i,j}}\right)^2$
...
So, we want to get a feele for how likely is it that we just got unlucky.  If the null hypothesis was true than how we labled our students (scent or noscent wouldn't matter)
...
Once we have made our additional asusmptions we can now easily find things like confidence intervals.  Most CIs have the form:
