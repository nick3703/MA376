...
classical<-music.dat%>% filter(MusicType=="Classical")
qqnorm(classical$Amount)
qqline(classical$Amount)
```
...
(24.9-21.7)+2.7*2.85*sqrt(1/142+1/131)
...
To check we can again use the formula for our book $\bar{y_i }\pm \frac{\hat{\sigma}}{\sqrt{n_i}}$.  

\vspace{.5in}

What, in this formula, changes as $i$ gets bigger?
