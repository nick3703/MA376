...
j & = \mbox{group}\\
i & = \mbox{observation}\\
y_{i,j}& = \mbox{response from the }i\mbox{th observation from the }j\mbox{th group}\\
\bar{y}& = \mbox{Overall sample mean}\\
\bar{y}_j &= \mbox{Sample mean of the jth group}\\
n &= \mbox{total sample size}\\
n_j &= \mbox{group size of the jth group}
\end{align*}
...
Note here we've put additional structure on $\mu$.  It would make sense to estimate $\hat{\mu_1}=\frac{1}{n_1}\sum_{i=1}^{n_i}y_{i,1}$ and $\hat{\mu_2}=\frac{1}{n_2}\sum_{i=1}^{n_i}y_{i,2}$.  Our Sum of square errors now is:
```{r}
...
$$\sum_{i,j}(\bar{y}_j-\bar{y})^2$$
If this number is really big, what can we say about our effect size?

\vspace{.4in}

Note that the calculations in some cases can be extremely simplified as $n_1=n_2$ we must have $\bar{y}_1$ and $\bar{y}_2$ to be equadistant from $\bar{y}$, so we end up with $\sum_{i,j}(\bar{y}_j-\bar{y})^2=n_1 \hat{\alpha}_1^2+n_2 \hat{\alpha}_2^2$.  Now this doesn't always hold true, so remember what we are doing is $\sum_{i,j}(\bar{y}_j-\bar{y})^2$ and we will be fine.  Note here we must have $\alpha_1+\alpha_2=0$ so our degrees of freedom are 1.

This calculation is called Sums of square due to model or SSM.
